...
export class ToolbarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.pageTitleDirection = this._layout.getProp('pageTitle.direction') as string;
  }

  ngAfterViewInit() {
    if (this.stPageTitle) {
      for (const key in this.pageTitleAttributes) {
        if (this.pageTitleAttributes.hasOwnProperty(key) && this.stPageTitle.nativeElement) {
