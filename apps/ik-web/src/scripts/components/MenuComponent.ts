...
import {
  DataUtil, DOMEventHandlerUtil, ElementStyleUtil,
  EventHandlerUtil, getAttributeValueByBreakpoint, getElementChild,
  getElementParents, getHighestZindex, getUniqueIdWithPrefix, insertAfterElement, slideDown, slideUp, throttle
} from '../_utils/index'
...
class MenuComponent {
  ...
  constructor(_element: HTMLElement, options: MenuOptions) {
    ...
    // this.element.setAttribute('data-st-menu', 'true')
  ...
  private _setTriggerElement = () => {
    const target = document.querySelector(
      `[data-st-menu-target="#${this.element.getAttribute('id')}"`
    )

    if (target) {
      this.triggerElement = target as HTMLElement
    } else if (this.element.closest('[data-st-menu-trigger]')) {
      this.triggerElement = this.element.closest('[data-st-menu-trigger]') as HTMLElement
    } else if (
      this.element.parentNode &&
      getElementChild(this.element.parentNode as HTMLElement, '[data-st-menu-trigger]')
    ) {
      const child = getElementChild(
        this.element.parentNode as HTMLElement,
        '[data-st-menu-trigger]'
      )
      if (child) {
  ...
  private _getItemOption = (item: HTMLElement, name: string) => {
    ...
    if (item && item.hasAttribute('data-st-menu-' + name)) {
      const attr = item.getAttribute('data-st-menu-' + name) || ''
  ...
  private _getItemElement = (_element: HTMLElement): HTMLElement | undefined => {
    ...
    if (_element.hasAttribute('data-st-menu-trigger')) {
      return _element
    }
    ...
    const item = _element.closest<HTMLElement>('.menu-item[data-st-menu-trigger]')
    if (item) {
      return item
    }
  ...
  private _getItemParentElement = (item: HTMLElement) => {
    ...
    const parentItem = sub.closest<HTMLElement>('.menu-item[data-st-menu-trigger]')
    if (sub && parentItem) {
      return parentItem
    }
  ...
  private _getItemChildElement = (item: HTMLElement): HTMLElement | null => {
    ...
    if (selector) {
      //element = selector.querySelector('.show.menu-item[data-st-menu-trigger]');
      const element = selector.querySelector<HTMLElement>('.menu-item[data-st-menu-trigger]')
      if (element) {
        return element
      }
    }
  ...
  private _hasItemSub = (item: HTMLElement) => {
    return item.classList.contains('menu-item') && item.hasAttribute('data-st-menu-trigger')
  }
  ...
  private _showDropdown = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'st.menu.dropdown.show') === false) {
      return
    }
    ...
    EventHandlerUtil.trigger(this.element, 'st.menu.dropdown.shown')
  }
  ...
  private _hideDropdown = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'st.menu.dropdown.hide') === false) {
      return
    }
    ...
    EventHandlerUtil.trigger(this.element, 'st.menu.dropdown.hidden')
  }
  ...
  private _showAccordion = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'st.menu.accordion.show') === false) {
      return
    }
    ...
    if (subElement) {
      ...
      slideDown(sub, this.options.accordion.slideSpeed, () => {
        ...
        item.classList.add('show')
        sub.classList.add('show')
        EventHandlerUtil.trigger(this.element, 'st.menu.accordion.shown')
      })
    }
  }

  private _hideAccordion = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'st.menu.accordion.hide') === false) {
      return
    }
    ...
    if (sub) {
      slideUp(sub, this.options.accordion.slideSpeed, () => {
        ...
        EventHandlerUtil.trigger(this.element, 'st.menu.accordion.hidden')
      })
    }
  }
  ...
  private _hideAccordions = (item: HTMLElement) => {
    const itemsToHide = this.element.querySelectorAll('.show[data-st-menu-trigger]')
  ...
  private _update = () => {
    const items = this.element.querySelectorAll('.menu-item[data-st-menu-trigger]')
  ...
  private _link = (element: HTMLElement, e: Event) => {
    if (EventHandlerUtil.trigger(this.element, 'st.menu.link.click') === false) {
      return
    }

    // Dismiss all shown dropdowns
    MenuComponent.hideDropdowns(undefined)
    EventHandlerUtil.trigger(this.element, 'st.menu.link.clicked')
  }
  ...
  // Get STMenu instance by element
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    const items = document.querySelectorAll<HTMLElement>(
      '.show.menu-dropdown[data-st-menu-trigger]'
    )
  ...
  public static updateDropdowns = () => {
    const items = document.querySelectorAll('.show.menu-dropdown[data-st-menu-trigger]')
    if (items && items.length > 0) {
  ...
  public static initGlobalHandlers = () => {
    ...
    document.addEventListener('click', (e) => {
      const menuItems = document.querySelectorAll('.show.menu-dropdown[data-st-menu-trigger]')
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '.menu-item[data-st-menu-trigger] > .menu-link, [data-st-menu-trigger]:not(.menu-item):not([data-st-menu-trigger="auto"])',
      'click',
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '.menu-item:not([data-st-menu-trigger]) > .menu-link',
      'click',
      function (this: HTMLElement, e: Event) {
        e.stopPropagation()
    ...
    window.addEventListener('resize', () => {
      ...
      throttle(
        ...
        () => {
          ...
          const elements = document.querySelectorAll('[data-st-menu="true"]')
          elements.forEach((el) => {
  ...
  public static bootstrap = () => {
    ...
    MenuComponent.createInstances('[data-st-menu="true"]')
  }
