import {
  DataUtil,
  ElementAnimateUtil, getAttributeValueByBreakpoint, getObjectPropertyValueByKey, getScrollTop, getUniqueIdWithPrefix, stringSnakeToCamel, throttle
} from '../_utils/index'
...
class ScrollTopComponent {
  ...
  private _scroll = () => {
    ...
    if (pos > offset) {
      if (!document.body.hasAttribute('data-st-scrolltop')) {
        document.body.setAttribute('data-st-scrolltop', 'on')
      }
    } else {
      if (document.body.hasAttribute('data-st-scrolltop')) {
        document.body.removeAttribute('data-st-scrolltop')
      }
    }
  }
  ...
  private _getOption = (name: string) => {
    const attr = this.element.getAttribute(`data-st-scrolltop-${name}`)
    if (attr) {
  ...
  public static bootstrap = () => {
    ScrollTopComponent.createInstances('[data-st-scrolltop="true"]')
  }
