...
const defaultImageInputQueires: IImageInputQueries = {
  ...
  instanseQuery: '[data-st-image-input]',
  inputQuery: 'input[type="file"]',
  wrapperQuery: '.image-input-wrapper',
  cancelQuery: '[data-st-image-input-action="cancel"]',
  removeQuery: '[data-st-image-input-action="remove"]',
...
class ImageInputComponent {
  ...
  private _change = (e: Event) => {
    ...
    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      // Fire change event
      if (EventHandlerUtil.trigger(this.element, 'st.imageinput.change', e) === false) {
        return
      }
      ...
      // Fire removed event
      EventHandlerUtil.trigger(this.element, 'st.imageinput.changed', e)
    }
  }
  ...
  private _cancel = (e: Event) => {
    ...
    // Fire cancel event
    if (EventHandlerUtil.trigger(this.element, 'st.imageinput.cancel', e) === false) {
      return
    }
    ...
    // Fire canceled event
    EventHandlerUtil.trigger(this.element, 'st.imageinput.canceled', e)
  }
  ...
  private _remove = (e: Event) => {
    ...
    // Fire remove event
    if (EventHandlerUtil.trigger(this.element, 'st.imageinput.remove', e) === false) {
      return
    }
    ...
    // Fire removed event
    EventHandlerUtil.trigger(this.element, 'st.imageinput.removed', e)
  }
