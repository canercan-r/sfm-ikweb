import {
  DOMEventHandlerUtil,
  ElementStyleUtil, EventHandlerUtil, getAttributeValueByBreakpoint, getCSS, getObjectPropertyValueByKey, getUniqueIdWithPrefix, stringSnakeToCamel, throttle
} from '../_utils/index'
...
class DrawerComponent {
  ...
  constructor(_element: HTMLElement, options: DrawerOptions) {
    ...
    this.name = this.element.getAttribute('data-st-drawer-name') || ''
    this.shown = false
  ...
  private _update = () => {
    ...
    // Reset state
    const hasBaseClass = this.element.classList.contains(`${this.options.baseClass}-true`)
    const bodyCanvasAttr = String(document.body.getAttribute(`data-st-drawer-${this.name}-`))

    if (hasBaseClass === true && bodyCanvasAttr === 'true') {
      this.shown = true
    } else {
      this.shown = false
    }
  ...
  private _getOption = (name: string) => {
    const attr = this.element.getAttribute(`data-st-drawer-${name}`)
    if (attr) {
  ...
  private _toggle = () => {
    if (EventHandlerUtil.trigger(this.element, 'st.drawer.toggle') === false) {
      return
    }
    ...
    EventHandlerUtil.trigger(this.element, 'st.drawer.toggled')
  }

  private _hide = () => {
    if (EventHandlerUtil.trigger(this.element, 'st.drawer.hide') === false) {
      return
    }

    this.shown = false
    this._deleteOverlay()
    document.body.removeAttribute(`data-st-drawer-${this.name}`)
    document.body.removeAttribute(`data-st-drawer`)
    this.element.classList.remove(`${this.options.baseClass}-true`)
    if (this.toggleElement != null) {
      this.toggleElement.classList.remove('active')
    }

    EventHandlerUtil.trigger(this.element, 'st.drawer.after.hidden')
  }

  private _show = () => {
    if (EventHandlerUtil.trigger(this.element, 'st.drawer.show') === false) {
      return
    }

    this.shown = true
    this._createOverlay()
    document.body.setAttribute(`data-st-drawer-${this.name}`, 'true')
    document.body.setAttribute('data-st-drawer', 'true')
    this.element.classList.add(`${this.options.baseClass}-true`)
    if (this.toggleElement !== null) {
      this.toggleElement.classList.add('active')
    }

    EventHandlerUtil.trigger(this.element, 'st.drawer.shown')
  }
  ...
  public static handleDismiss = () => {
    ...
    DOMEventHandlerUtil.on(document.body, '[data-st-drawer-dismiss="true"]', 'click', () => {
      /* @ts-ignore */
      const element = this.closest('[data-st-drawer="true"]')
      if (element) {
    ...
    window.addEventListener('resize', function () {
      ...
      throttle(
        ...
        () => {
          ...
          const elements = document.body.querySelectorAll('[data-st-drawer="true"]')
          elements.forEach((el) => {
  ...
  public static bootstrap = () => {
    DrawerComponent.createInstances('[data-st-drawer="true"]')
