import {
  DataUtil,
  DOMEventHandlerUtil, ElementAnimateUtil, ElementStyleUtil, EventHandlerUtil, getElementIndex,
...
class StepperComponent {
  ...
  constructor(_element: HTMLElement, options: IStepperOptions) {
    ...
    // Elements
    this.steps = this.element.querySelectorAll('[data-st-stepper-element="nav"]')
    this.btnNext = this.element.querySelector('[data-st-stepper-action="next"]')
    this.btnPrev = this.element.querySelector('[data-st-stepper-action="previous"]')
    this.btnSubmit = this.element.querySelector('[data-st-stepper-action="submit"]')
  ...
  private _goTo = (index: number) => {
    EventHandlerUtil.trigger(this.element, 'st.stepper.change')
    ...
    EventHandlerUtil.trigger(this.element, 'st.stepper.changed')
  }
  ...
  private initHandlers = () => {
    this.btnNext?.addEventListener('click', (e: Event) => {
      ...
      EventHandlerUtil.trigger(this.element, 'st.stepper.next', e)
    })

    this.btnPrev?.addEventListener('click', (e: Event) => {
      e.preventDefault()

      EventHandlerUtil.trigger(this.element, 'st.stepper.previous', e)
    })

    DOMEventHandlerUtil.on(this.element, '[data-st-stepper-action="step"]', 'click', (e: Event) => {
      e.preventDefault()
  ...
  private getStepContent = (index: number) => {
    const content = this.element.querySelectorAll('[data-st-stepper-element="content"]')
    if (!content) {
      return false
    }
  ...
  private refreshUI = () => {
    ...
    const elements = this.element.querySelectorAll(
      '[data-st-stepper-element="nav"], [data-st-stepper-element="content"], [data-st-stepper-element="info"]'
    )
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (
          ...
          element.getAttribute('data-st-stepper-element') === 'content'
        ) {
  ...
  public static bootstrap(attr: string = '[data-st-stepper]') {
