...
function getElementActualCss(el: HTMLElement, prop: any, cache: boolean) {
  ...
  if (!el.getAttribute('st-hidden-' + prop) || cache === false) {
    let value
    ...
    // store it in cache
    if (value !== undefined) {
      el.setAttribute('st-hidden-' + prop, value.toString())
      return parseFloat(value.toString())
    }
    return 0
  } else {
    // store it in cache
    const attributeValue = el.getAttribute('st-hidden-' + prop)
...
function getBreakpoint(breakpoint: string) {
  let value: number | string = getCSSVariableValue('--st-' + breakpoint)
  if (value) {
