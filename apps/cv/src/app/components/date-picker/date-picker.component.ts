...
export class DatePickerComponent implements OnInit, ControlValueAccessor {
  ...
  @Input() formatter = (date: Date) => {
    if (this.viewType === DATE_VIEW.HAFTALIK) {
      ...
      return `${_startDate.getDate()} ${'-'} ${_endDate.getDate()} ${this.monthFormatter.format(
        _startDate
      )}`;
    } else if (this.inputFormat == this.stringToDateFormat) {
      return `${date.getDate()} ${this.monthFormatter.format(date)},${this.dayFormatter.format(
        date
      )}`;
    } else {
      return format(date, this.inputFormat);
    }
  };
  ...
  constructor(
    ...
    readonly _lang: LanguageService
  ) {
    if (!ngCtrl) {
      throw new Error(
        'lib-date-picker component formcontrolname veya ngmodel olmadan kullanılamaz'
      );
    }
