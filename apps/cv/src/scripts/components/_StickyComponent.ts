import {
  ...
  ElementAnimateUtil,
  ElementStyleUtil,
  EventHandlerUtil, getAttributeValueByBreakpoint, getCSS, getElementOffset, getObjectPropertyValueByKey, getScrollTop, getUniqueIdWithPrefix, stringSnakeToCamel
} from '../_utils/index'
...
class StickyComponent {
  ...
  constructor(_element: HTMLElement, options: StickyOptions) {
    ...
    this.instanceName = this.element.getAttribute('data-st-sticky-name')
    this.attributeName = 'data-st-sticky-' + this.instanceName
    ...
    if (reverse === true) {
      ...
      if (st > offsetNum && this.lastScrollTop < st) {
        ...
        if (this.eventTriggerState === true) {
          EventHandlerUtil.trigger(this.element, 'st.sticky.on')
          EventHandlerUtil.trigger(this.element, 'st.sticky.change')
        ...
        if (this.eventTriggerState === false) {
          EventHandlerUtil.trigger(this.element, 'st.sticky.off')
          EventHandlerUtil.trigger(this.element, 'st.sticky.change')
  ...
  private getOption = (name: string) => {
    const dataStickyAttr = 'data-st-sticky-' + name
  ...
  public static bootstrap(attr: string = '[data-st-sticky="true"]') {
    StickyComponent.createInstances(attr)
  }

  public static reInitialization(attr: string = '[data-st-sticky="true"]') {
    StickyComponent.createInstances(attr)
  }
}
