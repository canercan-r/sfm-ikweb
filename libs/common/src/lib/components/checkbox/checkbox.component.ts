...
export class CheckboxComponent<T extends object> implements ControlValueAccessor, OnInit {
  ...
  constructor(
    ...
    if (!ngCtrl) {
      throw new Error('lib radio component formcontrolname veya ngmodel olmadan kullanılamaz');
    }
    ngCtrl.valueAccessor = this;
  }

  change(eventArgs: IChangeCheckboxEventArgs) {
    const data = eventArgs.owner
