...
export class SelectComponent<T extends object> implements ControlValueAccessor, OnInit {
  ...
  constructor(
    ...
    if (!ngCtrl) {
      throw new Error('lib select component formcontrolname veya ngmodel olmadan kullanılamaz');
    }
    ngCtrl.valueAccessor = this;
  }

  change(eventArgs: ISelectionEventArgs) {
    const data = eventArgs.newSelection.value
